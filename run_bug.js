...
const PREDEFINED_NATIONAL_SQUADS = {
    ...
    'JAPÓN ORION': {
        formation: '4-3-3',
        coach: 'coach_zhaojinyun',
        pitch: [
            'MARK EVANS', 'NATHAN SWIFT', 'JACK WALLSIDE', 'MANSABE', 'NOBOU',
            'JUDE SHARP', 'ICHINOSE', 'KIRA HIROTO',
            'AXEL BLAZE', 'HAZAKI', 'NOSAKA'
        ],
        bench: ['ENDOU MAMORU', 'GOUENJI', 'FROSTE', 'ASUTO', 'HAISAKI', 'HIROTO']
    },
    'JAPÓN GO': {
        formation: '4-3-3',
        coach: 'coach_kudou',
...
function saveTournament(tournament) {
    localStorage.setItem('animeTournament', JSON.stringify(tournament));
}

function loadTournament() {
    var raw = localStorage.getItem('animeTournament');
    if (!raw) return null;
    try { return JSON.parse(raw); } catch(e) { return null; }
}

function clearTournament() {
    localStorage.removeItem('animeTournament');
}
...
let r = t.rounds[0];
let pMatch = r.matches.find(m => m.home.isPlayer || m.away.isPlayer);
pMatch.played = true;
let playerIsHome = pMatch.home.isPlayer === true;
pMatch.result = { home: playerIsHome ? 1 : 0, away: playerIsHome ? 0 : 1 };
pMatch.winner = playerIsHome ? pMatch.home : pMatch.away;

simulateAIMatches(t);
console.log('Advanced?', advanceBracket(t));
console.log('Next round:', t.currentRound);
let next = getPlayerNextMatch(t);
console.log('getPlayerNextMatch:', next ? 'FOUND' : 'NULL');
